...
public class EditBookTitleAndAuthorScreen extends WindowScreen {
    ...
    public void initWidgets() {
        ...
        WTextBox title = t.add(theme.textBox(itemStack.get(DataComponentTypes.WRITTEN_BOOK_CONTENT).comp_2419().get(mc.shouldFilterText()))).minWidth(220).expandX().widget();
        t.row();

        t.add(theme.label("Author"));
        WTextBox author = t.add(theme.textBox(itemStack.get(DataComponentTypes.WRITTEN_BOOK_CONTENT).comp_2420())).minWidth(220).expandX().widget();
        t.row();
        ...
        t.add(theme.button("Done")).expandX().widget().action = () -> {
            ...
            WrittenBookContentComponent newComponent = new WrittenBookContentComponent(RawFilteredPair.of(title.get()), author.get(), component.comp_2421(), component.pages(), component.comp_2423());
