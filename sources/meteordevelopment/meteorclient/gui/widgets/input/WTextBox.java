...
public abstract class WTextBox extends WWidget {
    ...
    public boolean onMouseClicked(Click click, boolean doubled) {
        if (mouseOver) {
            if (click.button() == GLFW_MOUSE_BUTTON_RIGHT) {
                ...
                double relativeMouseX = click.comp_4798() - x + overflowWidth;
                double pad = pad();
    ...
    public boolean onKeyPressed(KeyInput input) {
        ...
        boolean control = MacWindowUtil.IS_MAC ? input.comp_4797() == GLFW_MOD_SUPER : input.comp_4797() == GLFW_MOD_CONTROL;

        if (control && input.comp_4795() == GLFW_KEY_C) {
            if (cursor != selectionStart || cursor != selectionEnd) {
                mc.keyboard.setClipboard(text.substring(selectionStart, selectionEnd));
            }
            return true;
        }
        else if (control && input.comp_4795() == GLFW_KEY_X) {
            if (cursor != selectionStart || cursor != selectionEnd) {
                mc.keyboard.setClipboard(text.substring(selectionStart, selectionEnd));
                clearSelection();
            }
        ...
        else if (control && input.comp_4795() == GLFW_KEY_A) {
            cursor = text.length();
            selectionStart = 0;
            selectionEnd = cursor;
        }
        else if (input.comp_4797() == ((MacWindowUtil.IS_MAC ? GLFW_MOD_SUPER : GLFW_MOD_CONTROL) | GLFW_MOD_SHIFT) && input.comp_4795() == GLFW_KEY_A) {
            resetSelection();
        }
        else if (input.comp_4795() == GLFW_KEY_ENTER || input.comp_4795() == GLFW_KEY_KP_ENTER) {
            setFocused(false);

            if (actionOnUnfocused != null) actionOnUnfocused.run();
            return true;
        }
        else if (input.comp_4795() == GLFW_KEY_TAB && completionsW != null) {
    ...
    public boolean onKeyRepeated(KeyInput input) {
        ...
        boolean control = MacWindowUtil.IS_MAC ? input.comp_4797() == GLFW_MOD_SUPER : input.comp_4797() == GLFW_MOD_CONTROL;
        boolean shift = input.comp_4797() == GLFW_MOD_SHIFT;
        boolean controlShift = input.comp_4797() == ((SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_ALT : MacWindowUtil.IS_MAC ? GLFW_MOD_SUPER : GLFW_MOD_CONTROL) | GLFW_MOD_SHIFT);
        boolean altShift = input.comp_4797() == ((SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_CONTROL : GLFW_MOD_ALT) | GLFW_MOD_SHIFT);

        if (control && input.comp_4795() == GLFW_KEY_V) {
            clearSelection();
        ...
        else if (input.comp_4795() == GLFW_KEY_BACKSPACE) {
            if (cursor > 0 && cursor == selectionStart && cursor == selectionEnd) {
                String preText = text;

                int count = (input.comp_4797() == (SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_ALT : MacWindowUtil.IS_MAC ? GLFW_MOD_SUPER : GLFW_MOD_CONTROL))
                    ? cursor
                    : (input.comp_4797() == (SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_CONTROL : GLFW_MOD_ALT))
        ...
        else if (input.comp_4795() == GLFW_KEY_DELETE) {
            if (cursor == selectionStart && cursor == selectionEnd) {
        ...
        else if (input.comp_4795() == GLFW_KEY_LEFT) {
            if (cursor > 0) {
                // sets the cursor to just after the next leftmost space
                if (input.comp_4797() == (SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_CONTROL : GLFW_MOD_ALT)) {
                    cursor -= countToNextSpace(true);
                    resetSelection();
                }
                // sets the cursor to the beginning of the text box
                else if (input.comp_4797() == (SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_ALT : MacWindowUtil.IS_MAC ? GLFW_MOD_SUPER : GLFW_MOD_CONTROL)) {
                    cursor = 0;
                    resetSelection();
                }
            ...
            else if (selectionStart != selectionEnd && selectionStart == 0 && input.comp_4797() == 0) {
                cursor = 0;
                resetSelection();
                cursorChanged();
            }
        ...
        else if (input.comp_4795() == GLFW_KEY_RIGHT) {
            if (cursor < text.length()) {
            ...
            else if (selectionStart != selectionEnd && selectionEnd == text.length() && input.comp_4797() == 0) {
                cursor = text.length();
                resetSelection();
                cursorChanged();
            }
        ...
        else if (input.comp_4795() == GLFW_KEY_DOWN && completionsW != null) {
        ...
        else if (input.comp_4795() == GLFW_KEY_UP && completionsW != null) {
            int currentI = getSelectedCompletion();
    ...
    public boolean onCharTyped(CharInput input) {
        ...
        if (filter.filter(text, input.comp_4793())) {
            clearSelection();
