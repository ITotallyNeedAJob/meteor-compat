...
public class NotebotCommand extends Command {
    ...
    private void onReadPacket(PacketEvent.Receive event) {
        if (event.packet instanceof PlaySoundS2CPacket sound && sound.getSound().comp_349().id().getPath().contains("note_block")) {
            if (ticks == -1) ticks = 0;
    ...
    private Note getNote(PlaySoundS2CPacket soundPacket) {
        ...
        if (noteLevel == -1) {
            error("Error while bruteforcing a note level! Sound: " + soundPacket.getSound().comp_349() + " Pitch: " + pitch);
            return null;
        }

        NoteBlockInstrument instrument = getInstrumentFromSound(soundPacket.getSound().comp_349());
        if (instrument == null) {
            error("Can't find the instrument from sound! Sound: " + soundPacket.getSound().comp_349());
            return null;
        }
