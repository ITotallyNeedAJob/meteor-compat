...
public class PotionTimersHud extends HudElement {
    ...
    public void tick(HudRenderer renderer) {
        ...
        for (StatusEffectInstance statusEffectInstance : mc.player.getStatusEffects()) {
            if (hiddenEffects.get().contains(statusEffectInstance.getEffectType().comp_349())) continue;
    ...
    public void render(HudRenderer renderer) {
        ...
        for (Pair<StatusEffectInstance, String> potionEffectEntry : texts) {
            Color color = switch (colorMode.get()) {
                case Effect -> {
                    int c = potionEffectEntry.left().getEffectType().comp_349().getColor();
    ...
    private String getString(StatusEffectInstance statusEffectInstance) {
        return String.format("%s %d (%s)", Names.get(statusEffectInstance.getEffectType().comp_349()), statusEffectInstance.getAmplifier() + 1, StatusEffectUtil.getDurationText(statusEffectInstance, 1, mc.world.getTickManager().getTickRate()).getString());
    }
