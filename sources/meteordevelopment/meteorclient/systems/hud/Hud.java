...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    ...
    // COMPAT Phase 1: default on (no GUI/editor to enable it; toggle command can switch off).
    ...
    public void init() {
        ...
        // COMPAT Phase 0: no built-in HUD elements.
    }

    public void register(HudElementInfo<?> info) {
        infos.put(info.name, info);

        // COMPAT Phase 1: no HUD editor screen exists, so a registered element
        // would never become visible. Auto-add one instance unless already present.
        ...
        if (!present) add(info, 4, 4, XAnchor.Left, YAnchor.Top);
    }
    ...
    public void resetToDefaultElements() {
        // COMPAT Phase 0: no default elements (built-ins removed).
    }
    ...
    private void onRender(Render2DEvent event) {
        ...
        if (mc.options.hudHidden || mc.debugHudEntryList.isF3Enabled()) return;
    ...
    private boolean shouldHideHud() {
        // COMPAT Phase 0: WidgetScreen (GUI) removed.
        return hideInMenus.get() && mc.currentScreen != null;
    }
