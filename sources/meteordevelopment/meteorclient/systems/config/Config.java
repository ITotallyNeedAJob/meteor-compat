...
public class Config extends System<Config> {
    ...
    // COMPAT: system init/load runs inside the MinecraftClient constructor, before the
    // window exists. Guard title updates so config load/reset can't NPE that early.
    private static void updateWindowTitleSafe() {
        if (mc != null && mc.getWindow() != null) mc.updateWindowTitle();
    }
    ...
    // COMPAT Phase 0 note kept: font setting restored with GUI batch (Fonts exists now).
    ...
    public final Setting<Boolean> customWindowTitle = sgVisual.add(new BoolSetting.Builder()
        ...
        .defaultValue(false)
        .onModuleActivated(setting -> updateWindowTitleSafe())
        .onChanged(value -> updateWindowTitleSafe())
        .build()
    );
