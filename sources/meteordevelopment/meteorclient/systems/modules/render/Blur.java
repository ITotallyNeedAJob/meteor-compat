...
public class Blur extends Module {
    ...
    public Blur() {
        ...
        // COMPAT: FBOs need the window + GPU, neither of which exists at client
        // entrypoint when modules are constructed. Created lazily on first
        // render (see ensureFbos) and on resolution changes.
    ...
    private void onRenderAfterWorld() {
        // COMPAT: create FBOs on first render; the window/GPU do not exist yet
        // when the module is constructed at client entrypoint.
        if (!ensureFbos()) return;
    ...
    // COMPAT: FixedUniformStorage needs the GPU, which does not exist at client
    // entrypoint when modules are constructed. Create on first render use.
    private static FixedUniformStorage<BlurUniformData> uniformStorage;

    private static FixedUniformStorage<BlurUniformData> uniformStorage() {
        if (uniformStorage == null) {
            uniformStorage = new FixedUniformStorage<>("Meteor - Blur UBO", UNIFORM_SIZE, 6);
        }
