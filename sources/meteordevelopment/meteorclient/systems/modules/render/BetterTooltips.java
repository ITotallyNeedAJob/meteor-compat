...
public class BetterTooltips extends Module {
    ...
    private void appendTooltip(ItemStackTooltipEvent event) {
        ...
        if (statusEffects.get()) {
            if (event.itemStack().getItem() == Items.SUSPICIOUS_STEW) {
                ...
                if (stewEffectsComponent != null) {
                    for (StewEffect effectTag : stewEffectsComponent.comp_2416()) {
        ...
        if (foodInfo.get() && event.itemStack().contains(DataComponentTypes.FOOD)) {
            ...
            event.appendStart(Text.literal(String.format("🍖 %d (💛 %.1f)", food.comp_2491(), food.comp_2492())).formatted(Formatting.GRAY));
        }
    ...
    private void getTooltipData(TooltipDataEvent event) {
        ...
        // Banner preview
            ...
            if (!event.itemStack.getOrDefault(DataComponentTypes.BANNER_PATTERNS, BannerPatternsComponent.DEFAULT).comp_2428().isEmpty()) {
    ...
    private MutableText getStatusText(StatusEffectInstance effect) {
        ...
        if (effect.getEffectType().comp_349().isBeneficial()) return text.formatted(Formatting.BLUE);
    ...
    public boolean shouldOpenContents(AbstractInput input) {
        if (input instanceof Click click) return openContents() && openContentsKey.get().matches(click.comp_4800());
