...
public class AutoArmor extends Module {
    ...
    private int getItemSlotId(ItemStack itemStack) {
        ...
        return itemStack.get(DataComponentTypes.EQUIPPABLE).comp_3174().getEntitySlotId();
    }
    ...
    private int getScore(ItemStack itemStack) {
        ...
        if (itemStack.contains(DataComponentTypes.ATTRIBUTE_MODIFIERS)) {
            ...
            for (AttributeModifiersComponent.Entry modifier : component.comp_2393()) {
                if (modifier.comp_2395() == EntityAttributes.ARMOR || modifier.comp_2395() == EntityAttributes.ARMOR_TOUGHNESS) {
                    double e = modifier.comp_2396().value();

                    score += (int) switch (modifier.comp_2396().comp_2450()) {
                        case ADD_VALUE -> e;
                        case ADD_MULTIPLIED_BASE -> e * mc.player.getAttributeBaseValue(modifier.comp_2395());
