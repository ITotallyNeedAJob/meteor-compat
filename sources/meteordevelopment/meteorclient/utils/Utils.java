...
public class Utils {
    ...
    // COMPAT: lazy (ProjectionMatrix2 ctor needs the GPU device, which does not
    // exist yet when Utils first loads at client entrypoint).
    private static ProjectionMatrix2 matrix;

    private static ProjectionMatrix2 matrix() {
        if (matrix == null) matrix = new ProjectionMatrix2("meteor-projection-matrix", -10, 100, true);
        return matrix;
    }
            ...
            for (int i = 0; i < nbt3.size(); i++) {
                ...
                if (slot.get() >= 0 && slot.get() < items.length) {
                    switch (StackWithSlot.CODEC.parse(mc.player.getRegistryManager().getOps(NbtOps.INSTANCE), compound.get())) {
                        case DataResult.Success<StackWithSlot> success -> items[slot.get()] = success.value().comp_4212();
    ...
    public static void removeEnchantment(ItemStack itemStack, Enchantment enchantment) {
        EnchantmentHelper.apply(itemStack, components -> components.remove(enchantment1 -> enchantment1.comp_349().equals(enchantment)));
    }
