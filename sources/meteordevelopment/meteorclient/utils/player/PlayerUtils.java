...
public class PlayerUtils {
    ...
    public static Vec3d getHorizontalVelocity(double bps) {
        ...
        boolean a = false;
        if (mc.player.input.playerInput.comp_3159()) {
            velX += forward.x / 20 * bps;
            velZ += forward.z / 20 * bps;
            a = true;
        }
        if (mc.player.input.playerInput.comp_3160()) {
            velX -= forward.x / 20 * bps;
            velZ -= forward.z / 20 * bps;
            a = true;
        }
        ...
        boolean b = false;
        if (mc.player.input.playerInput.comp_3162()) {
            velX += right.x / 20 * bps;
            velZ += right.z / 20 * bps;
            b = true;
        }
        if (mc.player.input.playerInput.comp_3161()) {
            velX -= right.x / 20 * bps;
            velZ -= right.z / 20 * bps;
            b = true;
        }
    ...
    public static float possibleHealthReductions(boolean entities, boolean fall) {
        ...
        if (entities) {
            ...
            if (mc.world.getEnvironmentAttributes().getAttributeValue(EnvironmentAttributes.BED_RULE_GAMEPLAY).comp_5138()) {
        ...
        if (fall) {
            // COMPAT: NoFall removed - always account for fall damage.
