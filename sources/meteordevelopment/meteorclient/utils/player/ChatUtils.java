...
public class ChatUtils {
    ...
    // Called manually from compat init (no ReflectInit in Phase 0).
    ...
    public static void sendMsg(int id, @Nullable String prefixTitle, @Nullable Formatting prefixColor, Formatting messageColor, String messageContent, Object... args) {
        // COMPAT: addons occasionally pass malformed format strings (e.g. Glazed's
        // PremiumTunnelBaseFinder). Fall back to the raw message instead of crashing.
        String formatted;
        try {
            formatted = String.format(messageContent, args);
        }
        catch (RuntimeException e) {
            MeteorClient.LOG.warn("Bad chat format string '{}', using raw message.", messageContent);
            formatted = messageContent;
        }
        MutableText message = formatMsg(formatted, messageColor);
    ...
    public static MutableText formatCoords(Vec3d pos) {
        ...
        // COMPAT Phase 0: Baritone click-to-goal removed.
        return coordsText;
    }
}
